...
public class TriggerDamage : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D col)
    {
        ...
        if(GameManager.Instance.healthContainer.ContainsKey(col.gameObject))//var health = col.gameObject.GetComponent<Health>();
        {
            var health = GameManager.Instance.healthContainer[col.gameObject];
            health.TakeHit(damage,gameObject);
        }

        if(isDestroyingAfterCollision&&col.GetComponent<Health>())
        {
            if (destroyer == null)
            {
                Destroy(gameObject);
